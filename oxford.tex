...
\begin{frame}[c]{Proposed Research}
  \begin{itemize}[<+->]
  ...
  \note{
    \begin{itemize}
      \item To sum up, my dissertation will address the following open research questions within the area of verifying the correctness of concurrent programs
      \item First, we'd like to efficiently perform the verification without an explicit enumeration of all schedules
      \item But, while we'd like to be efficient, we also want to be able to verify properties across threads with reasonable acurracty
      \item We also want to be practical: our analysis should be able to reason about programs running on real processors
      \item And finally, we want our analysis to be incremental: if the user makes a small change to their program, the analysis should quickly analyze only the parts relevant to the change.
      \item So, overall, my dissertation will create the first Incremental Thread-Modular Static Verifier for both Strong and Weak Memory.
    \end{itemize}
  }
\end{frame}
  ...
  \note{
    \begin{itemize}
      \item We answer these research questions in three steps, each building
        upon the next
      \item My dissertation will address these issues surrounding writing
        correct concurrent code by contributing three novel techniques each
