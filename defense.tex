...
\ifnum#2=1\relax
\node[align=center, visible on=#4] at ( $ (#1.north west)!0.5!(#1.center) $ ) {#3};
\else
\ifnum#2=2\relax
\node[align=center, visible on=#4] at ( $ (#1.north east)!0.5!(#1.center) $ ) {#3};
\else
\ifnum#2=3\relax
\node[align=center, visible on=#4] at ( $ (#1.south west)!0.5!(#1.center) $ ) {#3};
\else
\ifnum#2=4\relax
\node[align=center, visible on=#4] at ( $ (#1.south east)!0.5!(#1.center) $ ) {#3};
\fi\fi\fi\fi%
}
...
\ifnum#2=1\relax
\node[visible on=#4, yshift=-1.5cm] at ( $ (#1.north west)!0.5!(#1.center) $ ) {\tiny#3};
\else
\ifnum#2=2\relax
\node[visible on=#4, yshift=-1.5cm] at ( $ (#1.north east)!0.5!(#1.center) $ ) {\tiny#3};
\else
\ifnum#2=3\relax
\node[visible on=#4, yshift=-1.5cm] at ( $ (#1.south west)!0.5!(#1.center) $ ) {\tiny#3};
\else
\ifnum#2=4\relax
\node[visible on=#4, yshift=-1.5cm] at ( $ (#1.south east)!0.5!(#1.center) $ ) {\tiny#3};
\fi\fi\fi\fi%
}
...
\end{adjustwidth}
  ...
  \note{
    \begin{itemize}
      \item First, a quick recap about the overall problem I'm looking at in my
        dissertation.
  ...
  \note{
    \begin{itemize}
      ...
      \item Typically, the two processes executed where first the trusted
        process ran to completion, thereby re-enabling write protection,
...
\begin{frame}[c]{State Explosion}
  ...
  \note{
    \begin{itemize}
      ...
      \item There are so many schedules that a programmer writing a concurrent
...
\begin{frame}{Incremental Thread Modular Abstract Interpretation}
  \begin{itemize}[<+->]
    \item Weak-memory Aware Dependency Analysis
    \item Weak-memory Aware Change--Impact Analysis
    \item Incremental thread modular abstract interpreter
    \item Reduces runtime overhead
  \end{itemize}
  \note{
    \begin{itemize}
      \item Today, I will be presenting the last portion of my dissertation
        which focuses on incremental thread modular abstract interpretation
      \item Creating the incremental analysis required solving many novel
        problems
      \item First, we developed a new dependency analysis which can find
        dependencies within programs written assuming weak memory. Here, the
        main challenge was modeling the effect of concurrency synchronization
        primitives such as fences
      \item Then, we created a new change--impact analysis which is capable of
        identifying statements in the program which were involved with a
        modification
      \item Specifically, if the programmer changes some lines of code in the
        program we'd like to know the set of all statements which could be
        impacted by this change
      \item And finally, we show how to integrate this change--impact analysis
        into the thread-modular abstract interpreter
      \item Through experiments, we show that this incremental analysis reduces
        runtime overhead significantly in the presense of small to medium
        programmer modifications
    \end{itemize}
  }
\end{frame}

\begin{frame}[c]{Completed Research}
  \begin{itemize}[<+->]
    \item Verifying Concurrent Programs:
  \begin{itemize}
    \item Efficiently: no explicit enumeriation of all schedules
    \item Acurractly: still verify properties across threads
    \item Practically: for weak-memory models found on real processors
    \item Incrementally: small change $\rightarrow$ fast analysis
  \end{itemize}
  \end{itemize}
  \pause
  \vspace{1em}
  \begin{center}
    \huge
    Incremental Thread-Modular Static Verification for Strong and Weak Memory
  \end{center}
  \note{
    \begin{itemize}
      \item And, just to reiterate: my dissertation addresses the following
        open research questions within the area of verifying the correctness of
        concurrent programs
      \item First, we'd like to efficiently perform the verification without an
        explicit enumeration of all schedules
      \item But, while we'd like to be efficient, we also want to be able to
        verify properties across threads with reasonable acurracty
      \item We also want to be practical: our analysis should be able to reason
        about programs running on real processors
      \item And finally, we wanted our analysis to be incremental: if the user
        makes a small change to their program, the analysis should quickly
        analyze only the parts relevant to the change.
      \item So, overall, my dissertation created the first Incremental
        Thread-Modular Static Verifier for both Strong and Weak Memory.
    \end{itemize}
  }
\end{frame}
...
\note {
  Next, I'll go over examples motiviating the last chapter on the incremental
  analysis.
}
\end{frame}
...
\end{columns}
  \note{
    \begin{itemize}
      \item First, to the best of our knowledge no change-impact analysis, or
        dependency analysis, exists handling weak-memory, particularly the
...
\begin{frame}[c]{Weak-Memory Dependency Analysis}
  \begin{columns}[c]
    ...
    \begin{column}{0.5\textwidth}
      \begin{itemize}[<+->]
        \item What statements depend on \texttt{x = a * 5}?
        \begin{itemize}
          \item \texttt{x = a * 5} updates \texttt{x}
          \item \texttt{y = x + 10} uses \texttt{x}
          \item Therefore \texttt{y = x + 10} depends on \texttt{x = a * 5}
        \end{itemize}
      \end{itemize}
    \end{column}
  \end{columns}
  ...
  \note{
    \begin{itemize}
      \item To know which statements are affected by a change we need to
        identify statements which are affected by some other statement, also
        known as forward dependencies
      \item For example, consider the statement here where \texttt{x} is set to
        the value of \texttt{a} times five
      \item If we want to know which other statements in the program are
        impacted by this statement we simply look forward for all the uses of
        \texttt{x} in the program.
      \item For example, if we have a statement where \texttt{y} is set to
...
\begin{frame}[c]{Weak-Memory Dependency Analysis}
  \begin{columns}[c]
    ...
    \begin{column}{0.5\textwidth}
      \begin{itemize}[<+->]
        \item No variables referenced within a fence
        \item Fences \emph{cannot} be analyzed flow insensitively
        \item Problem: flow-sensitivity greatly decreases runtime with minimal
          effect on accuracy [Hardekopf 2009]
      \end{itemize}
    \end{column}
  \end{columns}
  \note{
    \begin{itemize}
      \item We just looked at how to calculate dependencies for non-concurrency
        related statements like addition and multiplication
      \item Intuitively, we should be able to apply the same idea for
        concurrency related statements like fences and memory barriers
      \item However, when we try to do this we immediately run into a problem:
        if we just look at a statement like a fence in isolation we have no
        notion of the variables either used or updated by the fence
      \item The reason for this is because a fence's semantics imply reasoning
        about the ordering of statements in the program.
      \item Specifically, in this example the fence ensures that the write to
        \texttt{x} is propagated across all threads before the subsequent write
        to \texttt{y}
      \item Because such statement ordering is baked into the fence's semantics
        they cannot be reasonably be analyzed flow-insensitively
      \item The issue with this is that flow-sensitivity within static analyses
        often severely harms the runtime performance of the analysis, with
        minimal increases in acurracy
      \item So, we would like to avoid flow-sensitivity whereever possible
    \end{itemize}
  }
\end{frame}

\begin{frame}[c]{Semi-Flow-Sensitive Dependency Analysis}
  \begin{columns}[c]
    \begin{column}{0.5\textwidth}
      \Huge
      \texttt{x = 10;}
    ...
    \begin{column}{0.5\textwidth}
      \begin{itemize}[<+->]
        \item Analyze non-concurrency constructs flow-insensitively
        \item Analyze concurrency synchronization flow-sensitively
        \item What statements depend on the \texttt{fence}?
        \begin{itemize}
          \item \texttt{y = x + 20} is a store/load reachable from the
            \texttt{fence}
          \item Therefore, \texttt{y = x + 20} depends on the \texttt{fence}
        \end{itemize}
      \end{itemize}
    \end{column}
  \end{columns}
  \note{
    \begin{itemize}
      \item To solve this issue of balancing runtime overhead and accuracy we
        propose what we call a semi-flow-sensitive analysis: we analyze only
        the concurrency constructs flow-sensitively, and do everything else
        flow-insensitively
      \item Specifically, we analyze non-concurrency synchronization, such as
        the writes to \texttt{x} and \texttt{y}, flow-insensitively; and we
        analyze the fence flow-sensitively
      \item I showed briefly before the reasoning behind deducing dependencies
        involving non-concurrency synchronization such as the updates to
        \texttt{x} and \texttt{y}
      \item We proceed in a similar deductive manner to reason about fences.
      \item For example, if we want to see which statements depend on the
        fence, we first look for those stores/loads which are reachable from
        the fence
      \item This includes the write to \texttt{y}, therefore we know that the
        write to \texttt{y} is dependent on the fence.
    \end{itemize}
  }
\end{frame}
...
\note{
  \begin{itemize}
    \item At this point, I've shown the high-level idea of how we can reason
      about dependencies, specifically those involving concurrency
      synchronization primitives
    \item Given this dependency-reasoning system we can then generate the graph
      of all dependencies between statements, which is known as the program
      dependency graph
    \item Here is the dependency graph for the initial program we looked at
      where a fence was modified to be a store--store memory barrier
    \item We'll return to this example in detail after providing more
      background, but note that given this graph we can easily see which
      statements are affected by a change by looking at reachability within
      this graph
    \item Specifically, we can see that the store--store memory barrier impacts
      the read of \texttt{flag1} guarding the subsequent assertion
    \item What remains to actually make use of this dependency information
      is to then subsequently integrate it into the abstract interpreter: the
      remainder of this presentation will show how this is done
  \end{itemize}
}
\end{frame}

\begin{frame}{Cooperative Analysis}
  \begin{columns}
    \begin{column}{0.5\textwidth}
      \begin{center}
        {\Large Non-Cooperative}
        \vspace{3em}

        \begin{tikzpicture}
          \tikzstyle{node}=[thread,font=\Large]
          \node [node]            (ci-noloop) {Change--Impact Analysis};
          \node [node, below=5em] (ai-noloop) {Abstract Interpretation};
          \draw [->, garr] (ci-noloop.south) -- (ai-noloop.north);
        \end{tikzpicture}
      \end{center}
    \end{column}
    \pause
    \begin{column}{0.5\textwidth}
      \begin{center}
        ...
        \begin{tikzpicture}
          \tikzstyle{node}=[thread,font=\Large]
          \node [node]            (ci-loop) {Change--Impact Analysis};
          \node [node, below=5em] (ai-loop) {Abstract Interpretation};
          \draw [->, garr] ($(ci-loop.south) + (-1.0em, 0)$)
                           -- ($(ai-loop.north) + (-1.0em, 0)$);
          \draw [->, garr] ($(ai-loop.north) + (1.0em, 0)$)
                           -- ($(ci-loop.south) + (1.0em, 0)$);
        \end{tikzpicture}
      \end{center}
    \end{column}
  \end{columns}
  \note{
    \begin{itemize}
      \item Finally, our incremental abstract interpretation is the first of
        what we call cooperative
      \item Traditionally, a lightweight analysis such as a change--impact
        analysis is run prior and used within a more heavyweight analysis, such
        as numerical abstract interpretatoin
      \item Other examples of this pattern are using an alias analysis before
        numerical abstract interpretation, or using a change--impact analysis
        before symbolic execution
      \item Overall, the gist of this is using a lightweight analysis as a
        preprocessing step before a more heavyweight analysis
      \item Our insight, however, is that this introduces inacuracies into the
        analysis: the lightweight change-impact analysis is unaware of the more
        heavyweight information within the numerical abstract interpretation
      \item Our solution to this is to compute the change-impact analysis
        simultaneously along side the numerical abstract interpretation 
      \item In this way, we allow the change-impact analysis to make use of the
        heavyweight numerical information thereby increasing the acurracy of
        not only the change--impact but the overall numerical analysis
    \end{itemize}
  }
\end{frame}

\section{Cooperative Incremental Thread Modular Abstract Interpretation}

\begin{frame}{Overview}
  \tableofcontents[currentsection]
\note {
  Next, I'll discuss in detail how we made our incremental analysis. 
}
\end{frame}
...
\begin{frame}[c]{Incremental Analysis}
  \begin{itemize}[<+->]
    \item Given previously analyzed program $P_1$ analyze new version $P_2$
    \item Ignore unimpacted regions 
    \item Reuse old analysis information
    \item Challenges:
      \begin{itemize}
        \item Identifying impacted statements?
        \item Integrating with thread-modular analyzer?
      \end{itemize}
  \end{itemize}
  \note{
    \begin{itemize}
      ...
      \item This brings several challenges which we addressed, specifically, it
        is unclear how to identify impacted statements in the program,
        particularly when dealing with weak-memory primitives
      \item For example, if the user modifies, or removes a memory fence which
        statements in the program are impacted
      \item We needed to find an efficient way to compute this across threads
      \item Also, it is unclear how to integrate the impacted-statement
        information into the thread-modular analyzer in a way that
        significantly reduces runtime
      \item Next, I'll go over these two points in more detail
    \end{itemize}
  }
\end{frame}
...
\begin{frame}[fragile]{Change-Impact Analysis}
  ...
  \note{
    \begin{itemize}
      ...
      \item Consider this example where we change a function call from calling
        func to calling coolFunc
      \item Given this change, we'd like to know other statements in the
        program impacted by this modification.
    \end{itemize}
  }
\end{frame}
...
\begin{frame}{Change-Impact Analysis}
  ...
  \note{
    \begin{itemize}
      ...
      \item This is done using the dependency analysis we briefly introduced
        previously
      \item Specifically, the graph shows both the data and control
        dependencies between statements
      \item If there is an edge from one node to another, it means the node at
        the head of the arrow is dependent on the one at the tail
...
\begin{frame}{Weak Memory Aware Dependency Analysis}
  \begin{itemize}[<+->]
    \item ``Standard'' data and control dependencies [Kennedy '02]
      \begin{itemize}
        \item Flow-insensitivity ensures soundness with threads
      \end{itemize}
    \item Flow-sensitive memory barrier/fence dependency analysis [new]
    \item Computed as a nested fixed point
  \end{itemize}
  \note{
    \begin{itemize}
      \item The first part of our incremental analysis is a weak-memory aware
        dependency analysis
      \item To do this, we first build upon standard prior work calculating
        data and control dependencies with sequential program constructs
      \item We reason about the soundness of these prior analyses in the
        presense of threads by ensuring they are flow-insensitive: they admit
        all possible thread schedules
      \item Next, we add to this a flow-sensitive reasoning of memory barriers
        and fences
      \item Note that both of these two dependency analyses are computed in a
        nested fixed point
      \item This means that dependencies found in one are considered in the
        other and vice versa
    \end{itemize}
  }
\end{frame}

\begin{frame}[c]{Weak Memory Aware Dependency Analysis}
...
, ampersand replacement=\&
  ...
  \node [node,align=center] (x5) { \texttt{x = 5}};
  \& \node [node,align=center] (z1) { \texttt{z = 1}}; \\
%
  \node [node,align=center] (y10) { \texttt{y = 10}};
  \& \node [node,align=center] (fright) { \texttt{fence}}; \\
%
  \node [mod,align=center] (fleft) { \texttt{SSMembar}};
  \& \node [node,align=center] (fl2) { \texttt{flag2 = 1}}; \\
%
  \node [node,align=center] (fl1) { \texttt{flag1 = 1}};
  \&  \\
};%end matrix
  \node [node,align=center,below=3mm of m] (if-f1) {\texttt{if (flag1)}};
  \node [node,align=center,below=3mm of if-f1] (as1) {\texttt{assert(x + y == 15)}};
  \node [node,align=center,below=3mm of as1] (if-f2) {\texttt{if (flag2)}};
  \node [node,align=center,below=3mm of if-f2] (as2) {\texttt{assert(z == 1)}};

  % data dependencies
  \draw [visible on=<2->, -latex] (x5.east) -| ([xshift=10mm]as1.north);
  \draw [visible on=<3->] (y10.east) -- ([xshift=23.8mm]y10.east);

  % Control dependnecies on assertion
  \draw [-latex, visible on=<4->] (if-f1) -- (as1);
  \draw [-latex, visible on=<4->] (if-f2) -- (as2);

  %% Fence dependencies
  \draw [visible on=<5->] (x5.west) -| ([xshift=-3mm]fleft.west);
  \draw [visible on=<5->, -latex] ([xshift=-3mm]fleft.west) -- (fleft.west);
  \draw [visible on=<6->, -latex] (y10.south) -- (fleft.north);

  \draw [visible on=<7->, -latex] (fleft.south) -- (fl1.north);

  % Remaining dependencies

  \draw [visible on=<8->, -latex] (fl1.south) |- (if-f1.west);
  
  \draw [visible on=<8->, -latex] (z1) -- (fright);
  \draw [visible on=<8->, -latex] (fright) -- (fl2);

  \draw [visible on=<8->, -latex] (fl2) |- (if-f2);

  \draw [visible on=<8->, -latex] ([xshift=4mm]fright.east) |- (as2.east);
  \draw [visible on=<8->] (fright.east) -- ([xshift=4mm]fright.east);

  \draw [visible on=<8->] ([xshift=4mm] z1.east) -- ([xshift=4mm]fright.east);
  \draw [visible on=<8->] (z1.east) -- ([xshift=4mm] z1.east);
  
  %\draw [visible on=<8->] (fleft.east) -- ([xshift=22.2mm]fleft.east);
...
\note{
  \begin{itemize}
    \item To see the dependency analysis more concretely we'll show how we
      build it from the initial example
    \item Here, we have threads one in the upper left, thread two in the upper
      right, and thread three in the bottom center
    \item First, we have the regular data dependencies, such as the dependency
      from the writes to \texttt{x} and \texttt{y} to its read in the
      assertion, as well as the control-dependencies guarding the assertions
    \item Next, we have the dependencies related to fences: these include
      stores and loads to shared memory before and after the fence
    \item For example, store--store memory barrier is dependent on both the
      write to \texttt{x}, as well as the write to \texttt{y}. Additionally,
      the write to \texttt{flag1} on the left is dependent on the memory
      barrier.
    \item The remainder of the dependencies are handled in a similar way
    \item This gives us all the dependencies in the example, thus providing the
      full program dependency graph
  \end{itemize}
}
\end{frame}

\begin{frame}{Types of Impact} 
  \begin{center}
    \Large%
    \textbf{May Impact}%
  \end{center}
  ...
  \begin{columns}
    \begin{column}{0.5\textwidth}
      \begin{center}
        \Large
        \visible<2->{\texttt{a = 10;}}

        \textcolor{matred400}{\texttt{x = a + 5;}}
      \end{center}
    \end{column}
    \begin{column}{0.5\textwidth}
      \begin{center}
        ...
        \begin{tikzpicture}
          \tikzstyle{node}=[thread,font=\Large]
          \node [node]            (mi-one) {\texttt{a = 10}};
          \node [node, below=2em] (mi-two) {\texttt{x = a + 5}};
          \draw [->, garr] (mi-one.south) -- (mi-two.north);
        \end{tikzpicture}
        }%
      \end{center}
    \end{column}
  ...
  \begin{columns}
    ...
    \begin{column}{0.5\textwidth}
      \begin{center}
        ...
        \begin{tikzpicture}
          \tikzstyle{node}=[thread,font=\Large]
          \node [node] (im-one) {\texttt{x = a + 5}};
          \node [node, below=2em]            (im-two) {\texttt{y = x * 10}};
          \draw [->, garr] (im-one.south) -- (im-two.north);
        \end{tikzpicture}
        }%
      \end{center}
    \end{column}
  ...
  \note{
    \begin{itemize}
      \item Given the weak-memory aware dependency analysis we then need to
        compute the change--impact analysis
      \item Again, the goal of the change--impact is to identify the regions of
        the program affected by a change
      \item We consider two different kinds of impact from a program
        modification
      \item The first considers statements which may-impact a modification
      \item For example, suppose we have the statement \texttt{x = a + 5} as
        being modified in the program
      \item The statements in the program which may-impact the modification are
        those which may, transitively or otherwise, affect the computation of
        the modification
      \item For example, if there is another statement \texttt{a = 10} in the
        program then this statement may-impact the modification
      \item Identifying these statements is done via backwards reachability on
        the program dependency graph, as seen here on the righ
      \item Dually, we consider statements in the program which may-be impacted
        by a modification to be those statements who's computation may-be
        affected by the modification
      \item Again, with the same modified statement, a statement such as
        \texttt{y = x * 10} may-be impacted by the modification
      \item These statements can be identified via forward reachability from
        the modified statement 
    \end{itemize}
  }
\end{frame}

\begin{frame}[c]{Weak-Memory Aware Change Impact Analysis}
\begin{center}
%%%% No nodes visible
\begin{tikzpicture}
  \tikzstyle{node}=[thread,font=\scriptsize]
...
\note{
  \begin{itemize}
    \item Given the previously described change--impact analysis, we can
      finally compute all the statements impacted by a modification
    \item Here we have the dependency graph of the example program, the
      modified statement is the store--store memory barrier highlighted in red
      on the left
  \end{itemize}
}
\end{frame}
...
\note{
  \begin{itemize}
    \item First, the statements which are forwardly reachable from the modified
      statement are those which we consider as impacted by the modification
    \item We can see those here outlined in dashed lines, including the write
      to flag1, and the read of flag one in the if condition, and the first
      assertion
  \end{itemize}
}
\end{frame}
...
\note{
  \begin{itemize}
    \item Additionally, the statements which are backwardsly reachable from the
      modified statement are those which may-impact the modification
    \item These include the writes to \texttt{x} and \texttt{y} before the
      store--store memory barrier
    \item This shows the final result of our change--impact analysis
    \item Here, we can intutively start see why the change-impact analysis can
      reduce the runtime of the numerical analysis: only a subset of the
      statements in the program are involved with the modification
  \end{itemize}
}
\end{frame}

\begin{frame}[c]{Incremental Thread-Modular Abstract Interpretation}
\begin{center}
 \begin{tikzpicture}
   \matrix (m) [
     matrix of nodes
    ...
    , ampersand replacement=\&
     ...
     \node [align=center] (np) {New Program Version ($P'$)};
     \& \node [align=center] (op) {Old Program Version ($P$)};
     \\
     \node [align=center] (bwi) {May-Impact Analysis};
     \& \node [align=center] (fwi) {May-be-Impacted Analysis};
     \\
     \node [align=center] (rtf) {Relevant \\ Transfer Functions};
     \& \node [align=center] (ps) {Preserved \\ Abstract States};
     \\
     \node [align=center] (tma) {Thread-Modular Analyzer \\ (Abstract Interpretation)};
     \& \node [align=center] (pr) {Proofs \\ (SC, TSO, PSO, RMO)};
     \\
   };% end matrix
    \draw[-latex] (np.south) -- (bwi.north);
    \draw[-latex] (np.south) -- ([yshift=-0.5mm, xshift=0.5mm]fwi.north west);
    \draw (np.west) -| ([xshift=-0.6cm] tma.west);
    \draw [-latex] ([xshift=-0.6cm] tma.west) -- (tma.west);

    \draw[-latex] (bwi.south) -- (rtf.north);
    \draw[-latex] (rtf.south) -- (tma.north);
    \draw[-latex] (op.south) -- (fwi.north);
    \draw[-latex] (op.south) -- ([yshift=-0.5mm,xshift=-0.5mm]bwi.north east);

    \draw (op.east) -| ([xshift=1.2cm] ps.east);
    \draw [-latex] ([xshift=1.2cm] ps.east) -- (ps.east);

    \draw[-latex] (fwi.south) -- (ps.north);
    \draw[-latex] (ps.south) -- ([yshift=-0.5mm,xshift=-0.5mm]tma.north east);
    \draw[-latex] (tma.east) -- (pr.west);
 \end{tikzpicture}
\end{center}
\note{
  \begin{itemize}
    \item Now that we've gone over our weak-memory aware change--impact
      analysis I'd like to take a moment to orient where we are in the big
      picture
    \item Recall that our goal was to perform an incremental analysis, that is,
      take an old, previously analyzed program version, and new program version
      and only analyze the portions of the new program which were modified
    \item We do this by first running the previously described may-impact and
      may-be impacted analyses on the programs to identify their respective
      statements
    \item The final part of the whole incremental analysis is to integrate the
      change--impact results, the may-impact and impacted analyses, into the
      thread-modular abstract interpreter
    \item We are able to reduce runtime overhead in two ways: first, we
      identify transfer functions which are relevant to the modifications; and
      second we preserve the analysis results from the old program version
      within the new progrma
    \item Care needs to be taken in both of these steps to ensure soundness
    \item The remainder of this presentation will describe briefly these two
      modifications
  \end{itemize}
}
\end{frame}
...
\note{
  \begin{itemize}
    \item The first part of integrating the change--impact analysis into the
      the abstract interpreter is identifying transfer functions in the program
      which are relevant given the modification
    \item Transfer functions within an abstract interpreter simply take some
      memory state in the program, and update the state with respect to a
      statement being executed
    \item For example, the transfer function for the first statement in
      thread one, setting \texttt{x} to 5, will take the state at the start of
      thread one and create a new state where \texttt{x} is 5.
    \item Here again we have the example program we've been looking at. Recall
      that the modification changes the fence to a store--store memory barrier
    \item Previously, we showed that the change--impact analysis could identify
      statements which were uninvolved with the modifications
    \item These statements can have their transfer functions ignored since they
      play no role in moving the old program version to the new program version
    \item Some of the most expensive parts of abstract interpretation are these
      transfer functions since they potentially require examining all the
      variables in the program
    \item Additionally, modifications to the program typically affect the data
      flow, that is, some set of variables in the program; but, the abstract
      interpreter analyzes the control-flow
    \item So, this can both prevent expensive operations, and overall better
      help the abstract interpreter match the problem being solved
    \item I'll skip the details now, but we show in our paper that this change
      is sound in that it will not cause any errors to be missed
  \end{itemize}
}
\end{frame}

\begin{frame}[fragile]{Preserving Analysis Results}
  \begin{columns}[c]
    \begin{column}{0.5\textwidth}
\begin{lstlisting}[language=C]
x = y = z = 0;
...
\note{
  \begin{itemize}
    \item The second integration with the abstract interpreter is to propagate
      the analysis results from the old program version to the new program
      version
    \item Abstract interpretation annotates the program with abstract memory
      states. Each state represents the set of all possible memory states on
      all executions at each statement in the program
    \item Here, I'm showing the states as these numbered S's  after each
      statement
    \item Since the entire goal of analyzing the new program version is to
      generate these states we'd like to be able to preserve them within the
      new program version
    \item We do this using the change--impact analysis by preserving the memory
      states at any statement which must not be impacted by the modification
    \item Intuitively, this is safe since the statements were not affected by
      the modification so their results will remain the same
    \item Again, we provide a more formal proof of this in my dissertation
  \end{itemize}
}
\end{frame}
...
\begin{frame}{Incremental Constraint-Based Abstract Interpreter for TSO, PSO, and RMO}
  ...
  \note{
    \begin{itemize}
      \item We compared against prior work made by Min\'{e} and implemented an
        incremental version of both his algorithm, as well as our
        constaint-based analyzer
...
\begin{frame}[c,fragile]{Effect of Size of Program Modification}
  \begin{center}
\begin{tikzpicture}[trim axis left]
  ...
  %  width=\textwidth-\widthof{R}-0.1cm,
  %  yticklabel style={align=right,inner sep=0pt,xshift=-0.1cm},
    xlabel={Impacted Statements (\%)},
    ylabel={Runtime (s)},
    legend entries={sc1200wdt},
  % ylabel={\scriptsize Runtime (s)},
    ...
    \addplot table {sc1200wdt.dat};
 \end{axis}
\end{tikzpicture}
  \end{center}
\note{
\begin{itemize}
  \item Finally, here is a little bit of information about the effect of the
    size of the modification made to the program on the runtime of the
    incremental analysis
  \item This graph shows the relationship between runtime and percentage of the
    program impacted by program changes for both the incremental and
    non-incremental technique
  \item We can see a somewhat linear relationship between the two showing that
    our technique is able to reduce runtime overhead when the program is not
    significantly modified
  \item However, as we approach 100\% of the program being modified the
    incremental approach is expected to be slightly slower than the
    non-incremental approach
  \item Note that these results only show the runtime relative to the
    percentage of impacted statements; our analysis also uses the percentage of
    statements which may-impact the change as well
  \item These are the results for the constraint based analysis but we saw
    similar results for the non-constraint based analysis as well
\end{itemize}
}
\end{frame}
...
\begin{frame}{Future Work}
  \begin{itemize}[<+->]
  \item More memory models: ARM, Power, C/C++, Java, interrupt/event-based
  \item More accurate constraints ($>$3 threads)
  \item Better alias analysis, more language features
  \item Different abstract domains
  \item Use a single solver (Datalog modulo theory, Prolog)
\end{itemize}
  \note{
\begin{itemize}
  \item Now, I'll start concluding by going over some ideas we have for future
    work
  \item First, we believe our constraint based analysis could be applied to
    other memory models such as ARM, Power, C/C++, Java, and interrupt or event
    based concurrency models. These would make the analysis more widely
    applicable, especially analyzing language level memory models
  \item Second, our technique starts to have inaccuracies since we only
    consider scenarios involving 2 threads, we believe we could reduce the
    number of false alarms by reasoning about more complex cases
  \item Third, it would be better if our implementation were more robust,
    meaning it had a more accurate alias analysis, and handled more of the
    language features
  \item Fourth, it would be interesting to apply the thread-modular abstract
    interpretation idea to non-thread modular abstract domains, for example, to
    analyze stuff like caches, worst case execution time, or worst case
    energy consumption
  \item Fifth, we explore various inter-thread data flow combinations which in
    the worst case is an NP-Complete problem so it would be best if we put the
    entire problem into a single solver, rather than using a boolean Datalog
    solver along side numerical domains
  \item in order to make use of advances in solver
    technology, we could use something like a datalog modulo-theory solver,
    prolog, or an SMT solver
\end{itemize}
  }
\end{frame}

\begin{frame}{Future Work: The Big Picture}
  \begin{itemize}[<+->]
    \item This work: refining an over-approximation
    \item Dual problem: refining an \emph{under}-approximation
    \item Can they be integrated?
    \item ``Solving'' the problem
      \begin{itemize}[<+->]
        \item Make the tool more robust
        \item Test on more programs
        \item Find more inaccuracies 
      \end{itemize}
    \item Does this technique generalize?
  \end{itemize}
  \note{
\begin{itemize}
  \item Next, I wanted to give a brief overview of the big picture of related
    techniques around this problem, and discuss how that relates to future work
  \item First, overall this technique worked on refining an over-approximation,
    that is, removing false alarms introduced by the over-approximation which
    correspond to behavior which does not actually exist in the program
  \item The dual problem is refining an under-approximation, that is,
    generalizing a model of the program in order to handle more of the actual
    behavior of the program. This includes techniques such as unit testing,
    model checking, and symbolic execution.
  \item In my mind, an open question for these dual techniques is if they can
    be combined and work together better than independently
  \item Related to this, is the question if the work in my dissertation reaches
    the end of this problem.
  \item We tested our tool on a bunch of real Linux device drivers, but it
    would be nice to expand the benchmarks to more types of programs.
  \item A big hurdle now is to make the tool more robust, and thus handle a
    larger variety of programs
  \item I believe testing on more programs would uncover more sources of
    inaccuracies, and thus new false alarms, that would require new challenges
    to address
  \item Finally, testing on a large number of programs would give a better idea
    if this technique generalizes and can be widely applicable
\end{itemize}
  }
\end{frame}
...
\begin{frame}{Conclusion}
  \begin{itemize}[<+->]
    \item Weak-memory aware dependency analysis
    \item Change--impact analysis for weak memory
    \item Incremental thread-modular numerical analysis
    \item Incremental thread-modular analysis for SC, TSO, PSO, RMO, and
      ``memory model oblivious''
  \end{itemize}
  \pause
  \begin{center}
  \Huge Thank You!
  ...
  \note{
    \begin{itemize}
      \item So, in conclusion I presented a weak-memory aware dependency
        analysis, and showed how to combine it into a change--impact analysis
        for weak memory 
      \item I then showed how we created an incremental numerical
        thread-modular abstract interpreter capable of skipping the transfer
        functions of some statements, as well as preserving the old analysis
        results in the new program version.
      \item This fits into my whole dissertation to build a thread-modular
        abstract interpretation analysis capable of analyzing SC, TSO, PSO,
        RMO, as well as prior memory model oblivious thread-modular analyses
      \item Finally, I wanted to thank everyone in my committee for being here
        today. I wouldn't be where I am today without the support of you and
        the university and I'm very fortunate for where I am now, so thanks :)
    \end{itemize}
  }
\end{frame}
...
\begin{frame}[c]
  \begin{center}
\begin{tikzpicture}
  \draw[fill=matorange400] (-5,-2) rectangle (5,0) node[pos=0.5, visible
  on=<4->] {Incremental
  Constraint Based Static Analysis};
  \draw[fill=matorange400] (-5,-4.5) rectangle (5,-2.5) node[pos=0.5, visible
  on=<3->] {Weak
  Memory Aware Constraint Based Static Analysis};
  \draw[fill=matorange400] (-5,-7) rectangle (5,-5) node[pos=0.5, visible
  on=<2->] {Constraint
...
\end{tikzpicture}
  ...
  \note{
    \begin{itemize}
      \item We answer these research questions in three steps, each building
        upon the next
      \item First, I show how a constraint sovler can be coupled with a numeric
        abstract interpreter to form a new kind of static analysis of
        concurrent progrmas
      \item Second, I show how this new analysis can be designed to acurratley
        analyze weak memory models
      \item And third, I show how this new weak-memory aware analysis can be
        made incremental.
      \item We've made significant progress already towards these goals, with
        the first two parts already published.
    \end{itemize}
  }
\end{frame}

\begin{frame}[fragile,t]{Related Work} 
\begin{center}
\begin{tikzpicture}
  ...
  \FillSquare{in}{1}{matorange400}
  \FillSquare{in}{2}{matorange400}
  \FillSquare{in}{3}{matorange400}
  \FillSquare{in}{4}{matorange400}

  \DrawLines{in}


  \LabelRegionVis{in}{1}{Bounded Model\\Checking}{<2->}
  \LabelRegionSmallVis{in}{1}{[Alglave et al, CAV '13]}{<2->}

  \LabelRegionVis{in}{2}{Memory Model Oblvious\\Thread Modular\\Static Analysis}{<3->}
  \LabelRegionSmallVis{in}{2}{[Min\'{e}, VMCAI '14]}{<3->}

  \LabelRegionVis{in}{3}{Non Modular\\Static Analysis}{<4->}
  \LabelRegionSmallVis{in}{3}{[Dan et al,  VMCAI '15]}{<4->}

  \LabelRegionVis{in}{4}{Incremental for SC}{<5->}
  \LabelRegionSmallVis{in}{4}{[Guo et al, FSE '15]}{<5->}
...
\note{
  \begin{itemize}
    \item We fill in the gaps and build on prior work in the following ways
    \item First, compared to prior work on bounded model checking, which
      attempts to find bugs in programs, we instead verify the absense of bugs,
      even in potentially infinite programs
    \item Second, prior thread-modular analyses are so called memory-model
      oblivious: these analyses are unaware of the underlying memory model and
      often admit incuarracies by assuming the memory model is too weak
    \item Third, are static verification techniques which are non-thread
      modular: these are inscalable due to their explicit enumeration of all
      thread schedules
    \item And fourth, are prior incremental analyses for concurrent programs:
      these only handled sequentially consistent programs, and are unable to
      model weak-memory concurrent programs.
  \end{itemize}
}
\end{frame}
...
\begin{tikzpicture}
  ...
  \LabelRegion{in}{1}{Test Generation}
  \LabelRegionSmall{in}{1}{[FSE15, ASE16]}

  \LabelRegion{in}{2}{Bug Hunting}
  \LabelRegionSmall{in}{2}{[ASE14, PLDI15, FSE16$^2$]}

  \LabelRegion{in}{3}{Repair}
  \LabelRegionSmall{in}{3}{[ICSE15, ISSTA15]}

  \LabelRegion{in}{4}{Verification}
  \LabelRegionSmall{in}{4}{[FSE16$^1$, FSE17, ASE17]}


  \node at (-2.5cm,3.5cm) {Incremental};
  \node at (-2.5cm,3.15cm) {\tiny [ASE16]};
  \node at (2cm,3.5cm) {Property Directed};
  \node at (2cm,3.20cm) {\tiny [ASE14, FSE15]};
...
\begin{frame}[fragile]{Flow-sensitive Thread-modular Abstract Interpretation}
  ...
  \only<8>{
    \begin{itemize}
      \item Load-store pairs: $(l_x, \Set{s_{x_1}, s_{x_2}})
                               , (l_f, \Set{s_{f}})
                              $%
    \end{itemize}%
  }%
  ...
  \note{
    \begin{itemize}
      ...
      \item First, we can see there are three interfering stores from thread
        one: two on x, setting it to four and five, and one on flag
...
\begin{frame}{Checking Interference Feasibility}
  ...
  \note{
    \begin{itemize}
      ...
      \item After building the must-happen before relation, any contradictions,
        i.e., cycles indicating a statement must happen-before itself
        indicating infeasibility.
...
\begin{frame}{Constraint-Based Abstract Interpreter for SC}
  ...
  \note{
    \begin{itemize}
      \item Here are the results of our tool testing sequentially consistent
        programs.
      \item We ran our tool on 38 C programs and compared it against Mine's
        non-constraint based thread-modular analyzer
      \item The benchmarks consisted of programs we created as well as linux
        device drivers
      \item His approach was able to verify 38 properties in 154 seconds,
        whereas our tool, with all it's optimizations, verified 1,078
        properties in 480 seconds
      \item This is a total of 28x more verified properties with a 3x increase
        in runtime
    \end{itemize}
  }
\end{frame}
...
\begin{frame}[c]{Memory Model Aware Static Analysis}
  \begin{center}
  \begin{tikzpicture}
  \matrix (mma) [matrix of nodes
                , column sep=20mm
                , row sep=10mm
                % Beamer messes with everything
                , ampersand replacement=\&
                , txt/.style={draw,minimum width=2cm,anchor=center,rounded
                corners,fill=matorange400,line width=1pt},
                ] 
    {
       {}       % m-1-1
       \& |[txt]| {\Large SC}% m-1-2
       \& |[txt]| {\Large Correct?}% m-1-2
       \\
       |[txt]| {\Large Program}% m-2-1
       \& |[txt]| {\Large PSO}% m-2-2
       \& |[txt]| {\Large Correct?}% m-2-2
       \\
       {}       % m-3-1
       \& |[txt]| {\Large RMO}% m-3-2
       \& |[txt]| {\Large Correct?}% m-3-2
       \\
    }; % matrix
      \draw[-latex,garr] (mma-2-1.east) -- (mma-1-2.west);
      \draw[-latex,garr] (mma-2-1.east) -- (mma-2-2.west);
      \draw[-latex,garr] (mma-2-1.east) -- (mma-3-2.west);

      \draw[-latex,garr] (mma-1-2.east) -- (mma-1-3.west);
      \draw[-latex,garr] (mma-2-2.east) -- (mma-2-3.west);
      \draw[-latex,garr] (mma-3-2.east) -- (mma-3-3.west);
  \end{tikzpicture}
  \end{center}
  \note{
    \begin{itemize}
      \item Specifically, we created a new framework where a static analysis
        can be parameterized by various memory models
      \item This has the advantage of the analysis reasoning about programs
        written for a specific computer architecture in mind
      \item For example, the user may perform certain optimizations which are
        only correct assuming a certain memory model
      \item Prior work is often unable to handle such a case: they are either too
        strong in that they miss potential bugs, or are too weak to verify the
        properties of interest
      \item Additionally, while we only considered TSO, PSO, and RMO in our
        current implementation, we believe additional memory models could be
        handled similarly
    \end{itemize}
  }
\end{frame}
...
\begin{frame}{Contributions}
  \begin{itemize}[<+->]
    \item Weak memory primitive semantics within numerical abstract
      interpretation
      \begin{itemize}
        \item CAS, TAS, fence, 
      \end{itemize}
    \item Infeasibility analysis for weak-memory
      \begin{itemize}
  ...
  \note{
    \begin{itemize}
      \item The specific contributions of this work included many proofs of
        correctness which for the sake of time I will not discuess in depth
        here
      \item But, in summary, we modeled the semantics of weak memory primitives
        within a numerical abstract interpretation, and show that this was
        correct
      \item We also created a new infeasibility analysis for weak-memory, and
        proved it was correct
      \item and finally, we evaluated our approach compared to prior work to
...
%  \LabelRegionSmallVis{in}{4}{[FSE16$^1$, FSE17, ASE17]}{<5->}
%
%
%  \node[visible on=<6->] at (-2.5cm,3.5cm) {Incremental};
%  \node[visible on=<6->] at (-2.5cm,3.15cm) {\tiny [ASE16]};
...
%  \LabelRegionSmall{in}{4}{[FSE16$^1$, FSE17, ASE17]}{<5->}
%
%
...
%\note{
%  \begin{itemize}
%    \item In this talk today, and in my dissertation proposal, I'll focus on my
%      current work on numerical verification, as well as the proposed ideas for
%      future work.
%  \end{itemize}
%}
%
  ...
  \note{
    \begin{itemize}
      ...
      \item First, the assertion in thread two is never violated: in order to
        ...
        means that the write to x of 5 from thread one must have already
        occurred 
...
\begin{frame}{Weak Memory Aware Dependency Analysis}
  \begin{itemize}[<+->]
    \item Dependencies calculated as system of deductive rules
    \item $s_1$: \texttt{x = 10;}
    \item $s_2$: \texttt{a = x + 10}
    \item \texttt{a} is dependent on \texttt{x}
    \item $s_1$ modifies \texttt{x}
    \item Therefore: $s_2$ is dependent on $s_1$
    \item Solvable in polynomial time
  \end{itemize}
\note{
  \begin{itemize}
    \item Briefly, I'll go over more of the details of how the dependency
      analysis is computed
    \item Overall, the dependencies are calculated using a system of deductive
      rules: repeatedly applying the rules until a fixed point is reached
      results in an overapproximation of all dependencies in the program
    \item As an example, suppose we have the following two statements, s1 and
      s2, where s1 writes to \texttt{x}, and s2 reads \texttt{x} and writes to
      \texttt{a}
    \item In this scenerio, we can see that \texttt{a} is dependent on
      \texttt{x}, and that statement $s_1$ modifies \texttt{x}
    \item From this, we can deduce that statement $s_2$ is dependent on $s_1$
    \item Here, we show a high-level idea behind the analysis. The low-level
      detail is that we express these high-level concepts as relations over
      statements and variables in the program
    \item Overall, the analysis ends up having worst case polynomial
      complexity
  \end{itemize}
}
\end{frame}
