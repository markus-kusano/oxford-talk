  ...
  \note{
    \begin{itemize}
      ...
      \item Because such statement ordering is baked into the fence's semantics
        they cannot be analyzed flow-insensitively without 
    \end{itemize}
  }
\end{frame}
...
\note{
  \begin{itemize}
    \item To reason about dependencies involving weak-memory primitives we plan
      to use what we call a semi flow-insensitive analysis: we perform the
      dependency analysis in a flow insensitive way except for with regards to
    ...
    \item Semantically, we consider a fence to be a flush of the store-buffers
      ...
      fence
    \item These semantics do not map to a flow-insensitive dependency analysis
      since, by definition, they involve knowing which statements ocurr before
      or after a memory barrier.
    \item To ensure soundness, some other peculiarties have to be delt with but
      For the sake of time, I wont go into the details but if you're
      interested I'd be happy to offline
    \item But, given the weak-memory aware dependency analysis we plan to
...
\begin{frame}{Overview}
  \tableofcontents[currentsection,currentsubsection]
\note {
  Before going into the three main bodies of work within my dissertation I'll
  provide some background necessary to understand my contributions.
}
\end{frame}
...
\section{Thread Modular Analysis for Weak Memory}

\begin{frame}{Overview}
  \tableofcontents[currentsection]
\note {
  Next, I'll go over the design of a thread-modular verification
  technique for weak memory.
}
\end{frame}
...
\section{Incremental Thread Modular Analysis}
