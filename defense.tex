  ...
  \note{
    \begin{itemize}
      \item To solve this issue of balancing runtime overhead and accuracy we
        ...
        the concurrency constructs flow-sensitively, and leave everything else
        as flow-insensitively
...
\note{
  \begin{itemize}
    ...
    \item What remains to actually make use of this dependency information is
      to then subsequently integrate it into the abstract interpreter: the
...
\note {
  Next, I'll discuss our proposed plan to make the previous thread-modular
  ...
  \note{
    \begin{itemize}
      ...
      \item Next, we add to this a flow-sensitive reasoning of memory barriers
        and fences
      \item Note that both of these analyses are computed in a nested fixed
        point
...
\begin{itemize}
  ...
  \item However, as we 100\% of the program being modified the incremental
    approach is slightly slower than the non-incremental approach
...
\begin{itemize}
  ...
  \item Fourth, it would be interesting to apply the thread-modular abstract
    ...
    energy consumption
  \item Fifth, we explore various interference combinations which in the worst
    case is an NP-Complete problem so it would be best if we put the entire
    problem into a single solver, rather than using a boolean Datalog solver
    along side numerical domains
